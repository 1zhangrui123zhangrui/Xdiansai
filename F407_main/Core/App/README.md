...
```c
圆1: [-236.8,  961.5, -189.5, -951.2]
圆2: [-934.0,  362.2,  924.6,  227.9]
圆3: [   0.0,    0.0,    0.0,    0.0]
圆4: [ 869.4,  364.7, -936.0,  250.5]
圆5: [-400.5, -850.0, -365.8,  900.0]
```
...
```c
(-20,  10): [  83.6,  743.0, -325.6, -651.4]
(-20,   0): [ 357.2,  543.0, -499.9, -380.0]
(-20, -10): [ 645.7,  436.9, -714.0,   -2.8]  // 已按 15 度少收线处理
( 20,  10): [-712.1,   -0.7,  578.7,  356.8]
( 20,   0): [-559.7, -331.7,  292.3,  530.9]
( 20, -10): [-478.4, -688.3,  -50.5,  721.6]  // 已按 15 度少收线处理
```

`(-20,-20)` 自动巡逻端点也按 15 度少收线处理，使用 `[854.4, 379.7, -951.0, 265.5]`。区域/顺序巡逻的圆4角度不受影响。
...
```text
(20,10)->(-20,10): ID1 若在收线, 前半段稍快；ID4 若在收线, 前半段更快；ID2/ID3 若在放线, 前半段稍慢
...
区域巡逻、顺序巡逻、自动巡逻的第一段默认按从圆3中心出发处理，因此从中心到第一个角点也会启用对应侧边电机曲线。
